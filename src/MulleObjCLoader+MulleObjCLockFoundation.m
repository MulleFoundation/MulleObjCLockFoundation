/*
 * template: project-oneshot/library/PROJECT_SOURCE_DIR/MulleObjCLoader+PROJECT_NAME.PROJECT_EXTENSION
 * vendor/extension: mulle-objc/objc
 */
#ifdef __MULLE_OBJC__

#import "MulleObjCLoader+MulleObjCLockFoundation.h"


@implementation MulleObjCLoader( MulleObjCLockFoundation)

//
// The file objc-loader.inc is generated by the mulle-objc-developer
// tool set automatically during a cmake build. Creation of this file can 
// be turned off in CMakeLists.txt with CREATE_OBJC_LOADER_INC=OFF
//
+ (struct _mulle_objc_dependency *) dependencies
{
   static struct _mulle_objc_dependency   dependencies[] =
   {
#include "objc-loader.inc"

      { MULLE_OBJC_NO_CLASSID, MULLE_OBJC_NO_CATEGORYID }
   };

   return( dependencies);
}

@end

#endif
